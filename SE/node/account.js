        ...
        return new Promise((res,rej)=>{
            db.collection('user').findOne({account:data.account},(err,result)=>{
                if(result){
                    ...
                    db.collection('user').insertOne(data,async(err,result)=>{
                        if(err)
                            rej(dbManipulationError);
                        else{
                            if(hasImage)
                                fs.renameSync(image.path,'../userImage/'+data.account+'.jpg');
                            await sendVerificationCodes(data['username'],data['email'],code);
                            res();
                        }
                    });
                }
        ...
        return new Promise((res,rej)=>{
            db.collection('user').findOne({account:account},(err,data)=>{
                if(err)
                    ...
                    db.collection('user').updateOne({account:account},{$set:{code:code}},async(err,result)=>{
                        if(err)
                            rej(dbManipulationError);
                        else{
                            await sendVerificationCodes(data['username'],data['email'],code);
